...
$stmt = $con->prepare("
    ...
    FROM leaves l 
    JOIN employees e ON (l.emp_id = e.eid OR l.emp_id = e.id)
    JOIN leave_hierarchy lh ON lh.employee_id = e.id
...
try {
    if ($action === 'recommend') {
        ...
        // For rejection, we need to check if user is either recommender or approver
        $stmt = $con->prepare("
            SELECT l.*, e.id as emp_numeric_id 
            FROM leaves l 
            JOIN employees e ON (l.emp_id = e.eid OR l.emp_id = e.id)
            JOIN leave_hierarchy lh ON lh.employee_id = e.id
            WHERE l.id = ? AND (lh.recommender_id = ? OR lh.approver_id = ?) AND l.status = 'Pending'
        ");
        $stmt->bind_param("iii", $leave_id, $recommender_id, $recommender_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $leave = $result->fetch_assoc();

        if (!$leave) {
            $_SESSION['error'] = "You don't have permission to reject this leave";
            header('Location: ../pending_leaves.php');
            exit;
        }

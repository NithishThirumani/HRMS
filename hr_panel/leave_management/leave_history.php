...
// Get leave history for all employees (HR view)
$leave_history = [];
if ($emp_data) {
    // For HR, show all leave applications (including those with invalid employee IDs)
    $query = "SELECT l.*, 
              ...
              LEFT JOIN departments d ON e.department_id = d.id
              LEFT JOIN employees rec ON l.recommender_id = rec.id
              LEFT JOIN employees app ON l.hod_id = app.id
    ...
    if ($stmt) {
        ...
        // Debug information
        error_log("HR Leave History Query executed. Found " . $count . " records.");
        if ($count == 0) {
            error_log("No leave records found in database.");
        }
    } else {
        $errors[] = "Error preparing leave history query: " . $con->error;
        error_log("Error in leave history query: " . $con->error);
    }
} else {
    error_log("Employee data not found for HR leave history.");
}
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Database Diagnostic</h6>
            </div>
            <div class="card-body">
                <?php
                // Check if leaves table exists and has data
                $check_query = "SELECT COUNT(*) as total_leaves FROM leaves";
                ...
                $emp_check_query = "SELECT COUNT(*) as total_employees FROM employees";
                $emp_check_stmt = $con->prepare($emp_check_query);
                $total_employees = 0;
                if ($emp_check_stmt) {
                    $emp_check_stmt->execute();
                    $result = $emp_check_stmt->get_result();
                    $row = $result->fetch_assoc();
                ...
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>Total Leave Records:</strong> <?php echo $total_leaves; ?></p>
                        <p><strong>Total Employees:</strong> <?php echo $total_employees; ?></p>
                        <p><strong>Leave History Records Found:</strong> <?php echo count($leave_history); ?></p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Current User:</strong> <?php echo htmlspecialchars($employee_name_display); ?></p>
                        <p><strong>User Department:</strong> <?php echo htmlspecialchars($department_name_display); ?></p>
                        <p><strong>Database Connection:</strong> <?php echo $con ? 'Connected' : 'Not Connected'; ?></p>
                    </div>
                </div>
                <?php if ($total_leaves > count($leave_history)): ?>
                <div class="alert alert-warning mt-3">
                    <strong>Data Integrity Issue Detected:</strong>
                    <p>There are <?php echo $total_leaves; ?> total leave records, but only <?php echo count($leave_history); ?> are showing. This suggests some records have invalid employee IDs (emp_id = 0 or non-existent employee IDs).</p>
                    <p><strong>Recommendation:</strong> Clean up the database by either:</p>
                    <ul>
                        <li>Updating invalid emp_id values to valid employee IDs</li>
                        <li>Deleting orphaned leave records</li>
                        <li>Fixing the leave application process to prevent invalid emp_id values</li>
                    </ul>
                </div>
                <?php endif; ?>
            </div>
        ...
        <?php if ($emp_data): // Only show content if employee data was loaded ?>
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">All Employees Leave Applications</h6>
                </div>
                <div class="card-body">
                            ...
                            <table class="table table-bordered" id="leaveHistoryTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($leave['recommender_name'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($leave['approver_name'] ?? 'N/A'); ?></td>
                                        <td><span class="badge badge-<?php echo getStatusBadgeClass($leave['status']); ?>"><?php echo htmlspecialchars($leave['status']); ?></span></td>
                                        <td><?php echo htmlspecialchars(date('d M Y H:i', strtotime($leave['applied_at']))); ?></td>
                                        <td>
                                            <a href="leave_slip.php?leave_id=<?php echo $leave['id']; ?>" target="_blank" class="btn btn-sm btn-outline-primary">
                    ...
                    <?php else: ?>
                        <div class="alert alert-info">
                            ...
                            <p>This could mean:</p>
                            <ul>
                                <li>No employees have applied for leave yet</li>
                                <li>The leaves table is empty</li>
                                <li>There might be a database connection issue</li>
                            </ul>
                            <p><strong>Debug Info:</strong> Check the error logs for more details.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>

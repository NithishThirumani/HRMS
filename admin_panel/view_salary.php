...
// Dynamic months array: from January to current month, add next month on last day
$currentYear = date('Y');
$currentMonth = date('n'); // 1-12
$today = date('Y-m-d');
$isLastDay = (date('Y-m-d', strtotime('last day of this month')) === $today);
$months = [];
for ($m = 1; $m <= $currentMonth; $m++) {
    $months[] = sprintf('%s-%02d', $currentYear, $m);
}
if ($isLastDay) {
    $nextMonth = $currentMonth + 1;
    if ($nextMonth <= 12) {
        $months[] = sprintf('%s-%02d', $currentYear, $nextMonth);
    }
}
...
if (isset($_POST['export_pdf'])) {
    ...
    <td>' . $row['others_deduction'] . '</td><td>' . $row['deduction_remarks'] . '</td><td>' . ($row['vendor_visa_remarks'] ?? 'N/A') . '</td><td>' . $row['performance_points'] . '</td><td>' . $row['visa_expense'] . '</td><td>' . $row['payable_salary'] . '</td><td>' . $row['deductions'] . '</td><td>' . $row['total_salary'] . '</td><td>' . ($row['bonus'] ?? '') . '</td></tr>';
        }
        $html .= '</table>';
    }
                            ...
                            <tbody>
                                <?php foreach ($salaries as $row): ?>
                                    <tr>
                                        ...
                                        <td><?php echo $row['vendor_visa_remarks'] ?? 'N/A'; ?></td>
